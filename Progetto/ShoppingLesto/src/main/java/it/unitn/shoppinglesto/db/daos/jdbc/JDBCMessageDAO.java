...
public class JDBCMessageDAO extends JDBCDAO<Message, Integer> implements MessageDAO {
    ...
    public List<Message> getNewMessages(Integer listId, Timestamp lastMessage) throws DAOException {
        List<Message> messages = new ArrayList<>();
        try (PreparedStatement preparedStatement = CON.prepareStatement("SELECT * FROM Message WHERE listId = ? AND createdAt > ? ORDER BY createdAT")) {
            preparedStatement.setInt(1, listId);
            preparedStatement.setTimestamp(2, lastMessage);
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    Message message = new Message();
                    message.setId(resultSet.getInt("id"));
